...
const htmlRegex =
  /^(?:(?:html ([a-z-]+)(?: element)?)|(?:([a-z-]+)(?: element)? html))$/i;

const apiRegex =
  /^(?:(?:(?:js |javascript )?([a-z ]+) api)|(?:([a-z ]+) (?:js |javascript )?api))$/i;
// "js closures"
const jsRegex =
  /^(?:(?:(?:js|javascript) ?([a-z ]+))|(?:([a-z ]+) (?:js|javascript)))$/i;
// "js what are closures"
const jsRegex2 =
  /^(?:(?:(?:js|javascript)(?: what( is|'s|s| are|'re))? ?([a-z ]+))|(?:what( is|'s|s| are|'re))? (?:([a-z ]+) (?:in )?(?:js|javascript)))$/i;
...
export async function request(query: string): Promise<EngineResponse> {
  ...
  if (match)
    return await makeSidebarResponse(`HTML/Element/${match[1] ?? match[2]}`);

  match = query.match(apiRegex);
  if (match)
    return await makeSidebarResponse(
      `API/${(match[1] ?? match[2]).replace(/ /g, "_")}`
    );

  match = query.match(jsRegex);
  if (match)
    return await makeSidebarResponse(
      `JavaScript/${(match[1] ?? match[2]).replace(/ /g, "_")}`
    );
