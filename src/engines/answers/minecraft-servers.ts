...
const minecraftRegex =
  /^(?:(?:minecraft|mc|server|ping|srv|serv|mine craft| )*?) *\b([-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}(?::\d{5})?)\b *(?:(?:minecraft|mc|server|ping|srv|serv|mine craft| )*?)$/i;
...
function isIp(string: string) {
  return /^((?:2[0-4]\d)|(?:25[0-5])|(?:1?\d?\d))\.((?:2[0-4]\d)|(?:25[0-5])|(?:1?\d?\d))\.((?:2[0-4]\d)|(?:25[0-5])|(?:1?\d?\d))\.((?:2[0-4]\d)|(?:25[0-5])|(?:1?\d?\d))$/.test(
    string
  );
}
...
export async function request(query) {
  ...
  else minecraftHost = regexMatch[1];
  let status;
  ...
  if (status) {
    const serverName = extractServerName(
      minecraftHost,
      flattenColorCode(status.description)
    );
    return {
      answer: {
        template: "minecraft",
