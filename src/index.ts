...
function loadTheme(name) {
  ...
  if (name !== "light")
    themeData = Object.assign(
      {},
      loadTheme(themeData.base || "light"),
      themeData
    );
  return themeData;
}
...
function render(
  res,
  template,
  options = {} as RenderOptions & Partial<Options>
) {
...
app.get("/autocomplete", async function (req, res) {
  ...
  res.header("Content-Type", "application/json").json([query, results]);
});
...
app.get("/plugins/:plugin", async function (req, res) {
  ...
  if (data === false)
    ...
    res.send("");
  else render(res, `plugins/${pluginName}.njk`, data);
});
...
app.get("/settings", function (req, res) {
  ...
  render(res, "settings.html", {
    themes,
    fonts: ["default", "Arial", "Monaco", "Poppins"],
    activeTheme,
    activeFont,
    debug: req.cookies.debug === "true",
    showIcons: req.cookies.showIcons === "true",
