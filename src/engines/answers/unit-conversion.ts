...
// day to second
const unitsInUnitRegex =
  /^(?:how many |number of )?(.+?)s? (?:(?:are )?in|to) (a|\d+) (.+?)s?$/i;
...
export async function request(query) {
  ...
  targetUnitAmount = isNaN(parseInt(targetUnitAmount))
    ? 1
    : parseInt(targetUnitAmount);
  ...
  let answer =
    (timeUnits[targetUnit] * targetUnitAmount) / timeUnits[currentUnit];
