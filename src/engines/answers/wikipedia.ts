...
export async function request(query: string): Promise<EngineResponse> {
  ...
  const summaryJson = await requestJSON(
    "https://en.wikipedia.org/w/api.php?format=json&action=query&prop=extracts|pageimages&exintro&explaintext&redirects=1&exsentences=2&titles=" +
      encodeURIComponent(wikipediaQuery)
  );
  ...
  if (article.extract.endsWith(":")) return {};
