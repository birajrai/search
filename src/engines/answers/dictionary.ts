...
async function dictionaryCom(query: string): Promise<DictionaryResponse> {
  return await parseDictionary(
    "https://www.dictionary.com/browse/" + encodeURI(query),
    {
      ...
      entryDefinitionsPath:
        "section > div > div[value], .default-content > div[value], h3 + div > div[value]",
...
function matchWord(query) {
  ...
  else if (!query.includes(" ") && !commonWords.includes(query)) return query;
  return {};
}
...
export async function request(query) {
  ...
  let { word, phoneticSpelling, ipaSpelling, entries, url } =
    await dictionaryCom(inputtedWord);
  if (!word) return {};
  return {
    answer: {
      template: "dictionary",
