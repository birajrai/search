...
export async function request(query) {
  ...
  const fromTimeString = `${fromHour % 12 || 12}:${("0" + fromMinute).slice(
    -2
  )}${fromAMPM}`;
  ...
  if (totalOffset - Math.floor(totalOffset) > 0) {
    const aheadBehindMinutes = Math.floor(
      (totalOffset - Math.floor(totalOffset)) * 60
    );
    aheadBehindString += `and ${aheadBehindMinutes} minutes `;
  }

  if (totalOffset >= 0) aheadBehindString += "ahead of ";
  else aheadBehindString += "behind ";
